...
namespace SoA.Content.Items.Accessories.UseAccessories
{
    public class COMusage : ModPlayer
    {
        ...
        private float lastLaserTime = -7f; // Время последнего использования лазера
        ...
        {
            float currentTime = Main.GameUpdateCount / 60f; // Текущее время в секундах
            if (currentTime - lastLaserTime >= 5f) // Меняется кулдаун
            {
                int numProjectiles = 5; // Количество выстрелов
        ...
        private void FireCursedProjectile()
        {
            ...
            if (target != null)
            {
                ...
                int damage = (int)(100 * Player.GetDamage(DamageClass.Magic).Multiplicative); // Меняется урон
        ...
        public override void UpdateBadLifeRegen()
        {
            if (CurseOfMe)
            {
                Player.lifeRegen = Math.Min(Player.lifeRegen, -3); // Меняется отрицательный реген
            }
        }
    }
}
